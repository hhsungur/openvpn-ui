...
// @router /certificates/send/:key [get]
func (c *CertificatesController) SendByMail() {
    c.TplName = "certificates.html"
    flash := web.NewFlash()

    cn := c.GetString(":key")

    logs.Info("Controller: SendByMail called with CN=%s", cn)

    cmd := exec.Command("/bin/bash", "/opt/scripts/sendmail.sh", cn)
    out, err := cmd.CombinedOutput()
    if err != nil {
        logs.Error("sendmail.sh failed: %v; out=%s", err, string(out))
    } else {
        flash.Success("Success! Mail has been sent to \"" + cn + "\"")
        flash.Store(&c.Controller)
    }
